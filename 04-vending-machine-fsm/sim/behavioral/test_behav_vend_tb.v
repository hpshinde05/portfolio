...
module vend_behav_tb();

    initial begin
        $dumpfile("test_behav_vend_tb.vcd");
        $dumpvars(0, vend_behav_tb);
    end
    ...
    lab3_behavioral uut(.x(x), .init(init), .clk(clk), .dispense(dispense), .change(change), .q(state));
